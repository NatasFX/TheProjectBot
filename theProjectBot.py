...
#histórico contendo também os warns, não somente os mutes.
#---------------------------------------------------------------------------------
...
setorajuda = {                                                                          #aqui é invocado quando alguém usa o $help para obter ajuda.
    ...
    '**$dice**': 'Joga dados. Truly random.',
    '**ping**': 'ping.',
    '**$tempmute**': 'Muta temporariamente usuários.',
    '**$unmute**': 'Desmuta usuários.',
    '**$tempwarn**': 'Limita usuários temporariamente.',
    '**$unwarn**': 'Remove limitação.',
    '**$pomodoro**': 'Temporizador para pomodoro em voice chat.',
    '**$histórico**': 'Mostra o seu histórico de mutes/warns ou de outro usuário',
...
infraçõesDeMembros = {}                                                                      #armazena pontos de infrações dos usuários, moderação
everyoneRole = ''
#---------------------------------------------------------------------------------------
...
class warning:
    def __init__(self, motivo, duração, admin):
...
class infração:
    def __init__(self, duração, peso, motivo, userID, admin):
        self.tempo = datetime.datetime.now()
        self.duração = duração
        self.motivo = motivo
        self.usuário = userID
...
def resetativo():                               #resetativo é o load das listas vulgo DB que o bot usa

    global userroles, direct_messages, saudações, initialtime, mutadosMembers, warnedMembers, membrosQuePrecisaDesmutar, infraçõesDeMembros
    userroles = loadlista(userroles, 'userroles')
    os.system('clear')
    print(f'\nloaded {len(userroles)} users and {sum([len(i)-1 for i in userroles.values()])} roles!\n')
    ...
    warnedMembers = loadlista(warnedMembers, 'warnedMembers')
    ...
    membrosQuePrecisaDesmutar = loadlista(membrosQuePrecisaDesmutar, 'membrosQuePrecisaDesmutar')
    print(f'Foram carregados {len(membrosQuePrecisaDesmutar)} membros que precisam ser desmutados.\n')

    infraçõesDeMembros = loadlista(infraçõesDeMembros, 'infraçõesDeMembros')
    print(f'Foram carregados {len(infraçõesDeMembros)} punidos e {sum([len(i) for i in infraçõesDeMembros.values()])} punições.\n')
...
def spammerdebomdialimiter(member):             #limitador de bom dia mas que eu uso em mts outros lugares tbm pra controle de fluxo aosdijasdoasd
    ...
    if member.id not in bomdiacooldown: bomdiacooldown.update({ member.id: datetime.datetime.now() }); return True

    if datetime.datetime.now() - bomdiacooldown[member.id] >= datetime.timedelta(seconds=20):
        del bomdiacooldown[member.id]
        return False
    else:
        return True
...
async def on_message(message):                                          #aqui ficam todos os comandos relacionados a mensagens enviadas, é chamado sempre que o bot consegue ver que foi enviado uma mensagem na guild
    ...
    global cargospegos, guild, member, ademir, mutedRole, cargo2pontos, cargocaveira, reg_tp, mirrorchannel, saudações, bate_papo_ids, direct_messages, limiter1dia, mutadosMembers, moderador, supervisor, warnRole, infraçõesDeMembros, everyoneRole
    ...
    if message.guild != None and cargospegos == False:
        ...
        everyoneRole = discordget(message.guild.roles, name="@everyone")
        cargospegos = True
    ...
    try:
        idDeMenção = message.mentions[0].id
    except:
        idDeMenção = 0


    if 'obrigado' in messagecontentlower[:8] or 'obrigada' in messagecontentlower[:8]:                   #se agradecer o bot, responder com uma mensagem legal
        ...
        elif not idDeMenção == 668640272573399041: return
        
        await message.channel.send(f'Disponha, {message.author.mention} {str("<:tplove:522462442366828574>")}'); return



    if 'Direct Message' in str(message.channel) and not message.author.bot and message.author.id != natasid:
        print(f'Received DM with {message.author} message=\'{message.content}\'')
        #time = currenttime()
        #savelista(direct_messages, 'direct_messages')
        #direct_messages.update({'testing':'my balls'})
        #savelista(direct_messages, 'direct_messages')
        bot_private = await client.fetch_channel(717215567169323028)
        try:
            await bot_private.send(f'[{currenttime()}]\tMensagem de {message.author.mention}: \n`{message.content}`')
        except:
            #await bot_private.send(f'<:tpwat:522462810676920322>\t[{currenttime()}]\tusuário {message.author.mention} enviou mensagem no privado mas era muito grande.\n`{message.content[:1900]}``')
            await bot_private.send(f'[{currenttime()}]\tMensagem de {message.author.mention}: \n`{message.content[:1900]}`')
    ...
    if extractor.has_urls(str(message.attachments)) and not message.author.bot and not 'Direct Message' in str(message.channel):
    ...
    #bate papo mirror
    if str(message.channel) == 'bate-papo' and not "mirror" in str(message.channel) and not 'Direct Message' in str(message.channel):
        global lastusersent
    ...
    #JULGAMENTO
    member = message.author
    if cargo2pontos in member.roles and message.channel.name != 'apresentações-introdução':                                #detecta se o user tem o cargo ..
        #pra não escutar introduçõe
        ...
        if len(message.content) > mensagemflood:                #se a mensagem é spam, contendo mais de 300 caracteres
            if member.top_role == cargocaveira:
                ...
                msg = await reg_tp.send(content=f'❌ [{currenttime()}] \n {ademir.mention}\n\nusuário {message.author.mention} mutado por comportamento potencialmente indesejado _(mensagem grande)_ no canal {message.channel.mention}\n\nMensagem enviada: {message.content[:1750]} \n\ntamanho: {len(message.content)}\\{mensagemflood} \n\n ')
                await message.channel.send(f'https://tenor.com/zH0p.gif', delete_after=60)
                if message.author.id in infraçõesDeMembros:
                    infraçõesDeMembros[message.author.id].append(
                        infração('Indefinida', 0, f'Mensagem longa. https://discordapp.com/channels/439263663837151242/675819847128711182/{msg.id}', message.author.id, 668640272573399041)
                    ); savelista(infraçõesDeMembros, "infraçõesDeMembros" )
                else:
                    infraçõesDeMembros.update({message.author.id: []})
                    infraçõesDeMembros[message.author.id].append(
                        infração('Indefinida', 0, f'Mensagem longa. https://discordapp.com/channels/439263663837151242/675819847128711182/{msg.id}', message.author.id, 668640272573399041)
                    ); savelista( infraçõesDeMembros, 'infraçõesDeMembros' )
        ...
        for i in imageextensions:                               #apaga mensagens contendo imagens como attachments
                ...
                if str(message.attachments).lower().find(i) != -1:  #procura se a mensagem tem imagem.
                    if member.top_role == cargocaveira:
                        ...
                        msg = await reg_tp.send(file=files, content='❌ [{}] \n {.mention}\n\n usuário {.mention} mutado por comportamento potencialmente indesejado _(envio de imagem)_ no canal {.mention} \n\ntrigger: {} \n\nMensagem enviada:\n\n'.format(currenttime(), ademir, message.author, message.channel, i))
                        tempmember = message.author
                        await message.channel.purge(limit=15, check=is_troller)
                        await message.channel.send(f'https://tenor.com/zH0p.gif', delete_after=60)
                        if message.author.id in infraçõesDeMembros:
                            infraçõesDeMembros[message.author.id].append(
                                infração('Indefinida', 0, f'Envio de Imagem. https://discordapp.com/channels/439263663837151242/675819847128711182/{msg.id}', message.author.id, 668640272573399041)
                            ); savelista(infraçõesDeMembros, "infraçõesDeMembros" )
                        else:
                            infraçõesDeMembros.update({message.author.id: []})
                            infraçõesDeMembros[message.author.id].append(
        ...
        for i in imageextensions:                               #apaga mensagens contendo link de imagens
                if messagecontentlower.find(i) != -1:
                    if member.top_role == cargocaveira:
                        ...
                        msg = await reg_tp.send(content='❌ [{}] \n {.mention}\n\n usuário {.mention} mutado por comportamento potencialmente indesejado _(envio de link de imagem)_ no canal {.channel} \n\ntrigger: {} \n\nMensagem enviada:\n\n{}'.format(currenttime(), ademir, message.author, message.channel, i, message.content))
                        #global tempmember
                        tempmember = message.author
                        await message.channel.purge(limit=15, check=is_troller)
                        if message.author.id in infraçõesDeMembros:
                            infraçõesDeMembros[message.author.id].append(
                                infração('Indefinida', 0, f'Envio de Imagem (link). https://discordapp.com/channels/439263663837151242/675819847128711182/{msg.id}', message.author.id, 668640272573399041)
                            ); savelista(infraçõesDeMembros, "infraçõesDeMembros" )
                        else:
                            infraçõesDeMembros.update({message.author.id: []})
                            infraçõesDeMembros[message.author.id].append(
    ...
    for i in redirects:                             #apaga mensagens contendo redirecionadores
        if messagecontentlower.find(i) != -1:
            if extractor.has_urls(message.content):
                ...
                if message.author.id in infraçõesDeMembros:
                    infraçõesDeMembros[message.author.id].append(
                        infração('Indefinida', 0, f'Link Redirect. https://discordapp.com/channels/439263663837151242/675819847128711182/{msg.id}', message.author.id, 668640272573399041)
                    ); savelista(infraçõesDeMembros, "infraçõesDeMembros" )
                else:
                    infraçõesDeMembros.update({message.author.id: []})
                    infraçõesDeMembros[message.author.id].append(
    ...
    if extractor.has_urls(message.content):                 #aqui verifica se os links enviador são verídicos, ou se tem problemas.
        for link in extractor.find_urls(message.content, check_dns=True, only_unique=True):
            ...
            for i in blacklist:                             #apaga mensagens contendo links apra sites pornográficos
                if link.lower().find(i) != -1:
                    ...
                    if message.author.id in infraçõesDeMembros:
                        infraçõesDeMembros[message.author.id].append(
                            infração('Indefinida', 0, f'Link dentro da Blacklist. https://discordapp.com/channels/439263663837151242/675819847128711182/{msg.id}', message.author.id, 668640272573399041)
                        ); savelista(infraçõesDeMembros, "infraçõesDeMembros" )
                    else:
                        infraçõesDeMembros.update({message.author.id: []})
                        infraçõesDeMembros[message.author.id].append(
            ...
            try:
                @timeout(5)
                def foo():
                    #if 'cdn.discordapp' in link or link[-4:] in ['.mp4', '.mov']: raise Exception("Ignoring this link.")
                    print(f"resolving {link}")
                    return requests.get(link, timeout=(5, 4)).text
                sitetext = foo()
            except Exception as e:
                print(f"Link enviado deu timeout. {e} Parando processo para não rolar exception no bot."); return
            if sitetext.lower().find('block this site') != -1 or 'www.rtalabel.org/index.php?content=parents' in sitetext:
                    await member.add_roles(mutedRole)
                    lista = []; [lista.append(role.name.replace('@everyone', f'{member.name}')) for role in member.roles]; userroles.update({member.id: lista}); savelista(userroles, 'userroles')
                    ...
                    if message.author.id in infraçõesDeMembros:
                        infraçõesDeMembros[message.author.id].append(
                            infração('Indefinida', 0, f'Site com string adulta. https://discordapp.com/channels/439263663837151242/675819847128711182/{msg.id}', message.author.id, 668640272573399041)
                        ); savelista(infraçõesDeMembros, "infraçõesDeMembros" )
                    else:
                        infraçõesDeMembros.update({message.author.id: []})
                        infraçõesDeMembros[message.author.id].append(
    ...
    if idDeMenção == 668640272573399041:
        if 'ajuda' in messagecontentlower or 'help' in messagecontentlower or messagecontentlower == '<@!668640272573399041>':
            ...
            embed.set_footer(text='Criado por Natas#9686')
    ...
    if spammerdebomdia and message.channel.name == 'bate-papo':
        if 'bom dia' in messagecontentlower and message.author.id not in saudações['bom dia'] and idDeMenção == 668640272573399041 or 'bom dia bot' in messagecontentlower and message.author.id not in saudações['bom dia']:#bomdia' in messagecontentlower.replace(' ', '') and spammerdebomdialimiter(message.author):
        ...
        if 'boa tarde' in messagecontentlower and message.author.id not in saudações['boa tarde'] and idDeMenção == 668640272573399041 or 'boa tarde bot' in messagecontentlower and message.author.id not in saudações['boa tarde']:#'boatarde' in messagecontentlower.replace(' ', '') and spammerdebomdialimiter(message.author):
        ...
        if 'boa noite' in messagecontentlower and message.author.id not in saudações['boa noite'] and idDeMenção == 668640272573399041 or 'boa noite bot' in messagecontentlower and message.author.id not in saudações['boa noite']:#'boanoite' in messagecontentlower.replace(' ', '') and spammerdebomdialimiter(message.author):
...
async def on_raw_reaction_add(event):                                   #add a role Clube do Livro/Grupode Estudos
    ...
    if event.channel_id == 697863869707845682:
        ...
        if not spammerdebomdialimiter(member):
            await event.member.send(f"{event.member.mention}\ncalma lá meu parceiro, vá mais com calma nas reações aí! Espere 30 segundos antes de tentar novamente.")
...
async def on_command_error(ctx, error):
    ...
    if 'tempmute' in ctx.invoked_with:
        await ctx.message.channel.send(f'{ctx.message.author.mention}, uso:\n$tempmute @user número[**m**inutos/**h**oras/**d**ias] [nº peso] motivo', delete_after=10); print(traceback.format_exc(), error )
...
async def join(ctx):                                                        #comando de música que é mt complicado implementar
    ...
    with ctx.message.channel.typing():
        if ctx.channel.name == 'bate-papo': await ctx.message.channel.send(f'{ctx.message.author.mention}, use este comando no chat de bots!')
...
@client.command()
async def repetir(ctx, channel: discord.TextChannel, *text):                                      #pra zuar
    
    message, author, channel, channelsend  =  ctx.message, message.author, message.channel, channel.send

    if channel.name != 'bots': await channelsend(f'{ctx.message.author.mention}, utilize o canal de bots por favor!', delete_after=10); return

    if not "Administrador" in str(author.roles) and author.id != natasid: await channelsend(f"{author.mention}, você não tem permissão para usar este comando!"); return
...
async def mensagemDM(ctx):                                                  #administradores podem usar o bot para enviar mensagens privadas para usuários
    ...
    await ctx.message.mentions[0].send(ctx.message.content[35:])
    await ctx.channel.send(f"{ctx.author.mention}, mensagem `{ctx.message.content[35:]}` enviada para {ctx.message.mentions[0]}!")
...
async def tempmute(ctx, muteMember: discord.Member, tempo: str, peso: str, *motivo):#muta algm temporariamente

    message = ctx.message
    author = message.author
    channelsend = message.channel.send
    #muteMember = message.mentions[0] muito bom poder setar os bgl junto do ctx.
    global mutedRole, reg_tp, mutadosMembers, ademir, moderador, supervisor, infraçõesDeMembros, infração
    ...
    if not peso.strip().isdigit():
        await channelsend(f'{author.mention}, especifique o peso da infração!\n$tempmute @user tempo [nº peso] motivo', delete_after=10); await message.delete(); return

    #criando objeto infração e adicionando no infraçõesDeMembros

    pesoMute = int(peso.strip())                                  #operadores ternários são pica


    try:

        if muteMember == '': await channelsend(f'{author.mention}, especifique um usuário!', delete_after=8); await message.delete(); return
        ...
        if muteMember.id in infraçõesDeMembros:
            soma = 0
            for infracao in infraçõesDeMembros[muteMember.id]:
                soma += infracao.peso
        else:
            infraçõesDeMembros.update({muteMember.id: []})
            soma = 0
        

        if soma + pesoMute >= 10:
            if mutedRole not in muteMember.roles:
                await muteMember.add_roles(discordget(message.guild.roles, name='Exilado'))     #tirando acesso dos canais
                ...
                embed = discord.Embed(
                    ...
                    description = f'**Usuário com 10{ "+" if soma > 10 else "" } pontos foi removido do bate-papo e shitpost**.\n⠀⠀\nMotivo: \n**{" ".join(motivo)}**\n⠀⠀\nÉ triste chegarmos neste ponto, {muteMember.name}. Repense seus atos, não gostamos de fazer isto.',
                ...
                await reg_tp.send(f"🤫\t[{currenttime()}]\t{muteMember.mention} foi mutado e removido dos chats bate-papo e shitpost por \"{tempo}\". motivo: \"`{' '.join(motivo)}`\"", allowed_mentions=discord.AllowedMentions(users=False))
    
                mutadosMembers.update({muteMember.id:datetime.datetime.now()+tempoDeMute}); savelista(mutadosMembers, 'mutadosMembers')
                infraçõesDeMembros[muteMember.id].append(
                    infração(tempoDeMute, pesoMute, " ".join(motivo), muteMember.id, author.id)); savelista(infraçõesDeMembros, 'infraçõesDeMembros')
                
                print(f'[{currenttime()}] a pedido de {author}, usuário {muteMember} mutado por {tempoDeMute}'); await message.delete(); return
            else:
                await channelsend(f'{author.mention}, usuário {muteMember.mention} já está mutado.', delete_after=5); await message.delete(); return
        
        elif mutedRole not in muteMember.roles:
            infraçõesDeMembros[muteMember.id].append(
                infração(tempoDeMute, pesoMute, " ".join(motivo), muteMember.id, author.id)); print(f'saved. {infraçõesDeMembros[muteMember.id][-1].tempo}') if savelista(infraçõesDeMembros, 'infraçõesDeMembros') else ''
            await muteMember.add_roles(mutedRole)
            lista = []; [lista.append(role.name.replace('@everyone', f'{muteMember.name}')) for role in muteMember.roles]; userroles.update({muteMember.id: lista}); savelista(userroles, 'userroles')

            embed = discord.Embed(
                title = f'🤫 {author.name}, usuário {muteMember} mutado!',
                description = f'⠀⠀\nMotivo: \n**{" ".join(motivo)}**\n\npontos: **{soma+pesoMute}**\n\n{author.mention} {muteMember.mention}',
                colour = discord.Color.red())
                
            embed.set_footer(text=f'Mutado até {(datetime.datetime.now()+tempoDeMute).strftime("%d/%m/%Y às %T")}')
            await channelsend(embed = embed)
            await reg_tp.send(f"🤫\t[{currenttime()}]\t{muteMember.mention} foi mutado por \"{tempo}\". pontos: {soma+pesoMute} motivo: \"`{' '.join(motivo)}`\"", allowed_mentions=discord.AllowedMentions(users=False))
            mutadosMembers.update({muteMember.id:datetime.datetime.now()+tempoDeMute}); savelista(mutadosMembers, 'mutadosMembers')
            print(f'[{currenttime()}] a pedido de {author}, usuário {muteMember} mutado por {tempoDeMute}'); await message.delete()
        else:
            await channelsend(f'{author.mention}, usuário {muteMember.mention} já está mutado.', delete_after=5); await message.delete(); return

    except:
        await channelsend(f'{author.mention}, uso:\n`$tempmute @user número[h/d/m] [nº peso] motivo`', delete_after=15); await message.delete()
...
async def unmute(ctx, user: discord.Member=''):                             #desmutar alguém né
    ...
    if mutedRole in user.roles:
        ...
        if user.id in mutadosMembers.keys():
...
async def pomodoro(ctx, comando: str, *args1):                              #comando pomodoro lindo
    ...
    author = message.author
    channelsend = message.channel.send
    global voicechannel, reg_tp, interromperLoop, pomodoroID, canalDePomodoro, pomodoroMemberCommand, everyoneRole
    ...
    if comando.lower() in ['iniciar', 'começar']:
        ...
        if len(args[0]) >= 3: await channelsend(f'{author.mention}, utilize o formato {"".join(filter(str.isdigit, list(args[0])))}m para representar {"".join(filter(str.isdigit, list(args[0])))}.', delete_after=15); return
        if len(args[1]) >= 2: await channelsend(f'{author.mention}, utilize o formato {"".join(filter(str.isdigit, list(args[1])))}m para representar {"".join(filter(str.isdigit, list(args[1])))}.', delete_after=15); return
        ...
        try:
            ...
            if channel.name not in ['Estudos III', 'Estudos II', 'Estudos']: await channelsend(f'{author.mention}, escolha um canal de estudos para iniciar o pomodoro!'); return
        ...
        currentID, canalDePomodoro, interromperLoop  =  pomodoroID, channel, False; print(f'started new pomodoro. ID: {currentID}');       
        ...
        while True:
            try:                                                                                                #usei um try mas não era estritamente necessário
                ...
                os.system('ffmpeg -i \"iniciando1.mp3\" -acodec copy \"iniciando.mp3\" -y -hide_banner -loglevel panic')     #corrigimos o bug que o ffmpeg não consegue determinar corretamente a duração do áudio aqui
                ...
                if pomodoroID != currentID: print(f'pomodoroID {currentID} finalizado.'); break                           #PARA NÃO BUGAR
                if client.voice_clients == [] or interromperLoop or len(channel.members) <= 1: await channel.set_permissions(everyoneRole, speak=None); await voicechannel.disconnect(); await channel.edit(name=f'{channel.name[:-14]}') if channel.name.find('[') != -1 else ''; break

                #INICIANDO POMODORO
                gtts(text=f'Iniciando pomodóro de {args[0]} minutos.', lang='pt-BR').save('iniciou1.mp3')
                os.system('ffmpeg -i \"iniciou1.mp3\" -acodec copy \"iniciou.mp3\" -y -hide_banner -loglevel panic')
                ...
                await discord.VoiceChannel.set_permissions(channel, everyoneRole, speak=False, connect=False)
                ...
                os.system('ffmpeg -i \"terminando1.mp3\" -acodec copy \"terminando.mp3\" -y -hide_banner -loglevel panic')
                ...
                os.system('ffmpeg -i \"descanso1.mp3\" -acodec copy \"descanso.mp3\" -y -hide_banner -loglevel panic')
                ...
                await discord.VoiceChannel.set_permissions(channel, everyoneRole, speak=None, connect=None)
        ...
        if pomodoroMemberCommand != author.id and author.id != natasid: await channelsend(f'Somente quem iniciou o pomodoro pode pará-lo!'); return
        pomodoroMemberCommand, interromperLoop, channel  =  '', True, message.guild.me.voice.channel

        await channel.set_permissions(everyoneRole, speak=None)
        
        if channel.name.find('[') != -1: await channel.edit(name=f'{channel.name[:-14]}')

        for mutado in channel.members:
            if mutado.bot or mutado.voice.mute == False: continue
        ...
        await channelsend(f'Gostou do pomodoro? Tem algum feedback/sugestão para o bot? Mande uma mensagem para {natasid.mention}', allowed_mentions=discord.AllowedMentions(users=False))
    else:
...
async def tempwarn(ctx, warnMember: discord.Member, tempo: str, *motivo):   #muta algm temporariamente
    ...
    try:
        ...
        global warnRole, reg_tp, warnedMembers, ademir, moderador, supervisor, natasid, warning, infraçõesDeMembros
        ...
        if warnRole not in warnMember.roles:
            ...
            if warnMember.id in infraçõesDeMembros:
                infraçõesDeMembros[warnMember.id].append(
                    warning(" ".join(motivo), tempoDeMute, author.id)); savelista(infraçõesDeMembros, 'infraçõesDeMembros')
                
            else:
                infraçõesDeMembros.update({warnMember.id: []})
                infraçõesDeMembros[warnMember.id].append(
                    warning(" ".join(motivo), tempoDeMute, author.id)); savelista(infraçõesDeMembros, 'infraçõesDeMembros')
...
@client.command(aliases = ['history', 'historico'])
async def histórico(ctx, *member_):
    
    message = ctx.message
    author = message.author
    channelsend = message.channel.send

    global infraçõesDeMembros, ademir, moderador, supervisor
    
    if not member_:
        member = author
    else:

        member = await message.guild.fetch_member(int(''.join(list(filter(str.isdigit, member_[0])))))  #isso tudo é só pra transformar a string de menção em um user válido
        
        if member.id == None:
            await channelsend(f'{author.mention}, especifique um usuário válido!', delete_after=15); message.delete(); return
        if not [cargo for cargo in [ademir, moderador, supervisor] if cargo in author.roles] and not author.id == natasid or author.id != member.id and not author.id == natasid:
            await channelsend(f'{author.mention}, você não tem permissão para usar este comando!'); return

    if member.id in infraçõesDeMembros:
        
        embed = discord.Embed(
            title = f'Histórico de {member}',
            description = f'**{author.name}**, aqui estão as informações do user!\n\n'+
                "\n\n".join(
                    f'[**{infrações.tempo.strftime("%d/%h/%y %H:%M")}** até **{(infrações.tempo+datetime.timedelta(days = infrações.duração.days, seconds=infrações.duração.seconds)).strftime("%d/%h/%y %H:%M") if str(infrações.duração) != "Indefinida" else "Indefinido"}**]:\n⠀⠀Tipo: **{infrações.tipo}** {"Mute por bot não adiciona ponto." if infrações.peso == 0 and infrações.tipo == "Mute" else "Warnings não implicam pontos." if infrações.tipo == "Warning" else f"Peso: {infrações.peso}"}\n⠀⠀Duração: {infrações.duração}\n⠀⠀Motivo: "{"_Motivo não especificado_" if infrações.motivo == "" else infrações.motivo}"\n⠀⠀Juiz: <@{infrações.admin}>' for infrações in infraçõesDeMembros[member.id])+
                f'\n\n{member.name} está com **{sum([i.peso for i in infraçõesDeMembros[member.id]])}** pontos{" e está Exilado." if "Exilado" in str(member.roles) else " e **deveria** estar exilado, ué??" if sum([i.peso for i in infraçõesDeMembros[member.id]]) >= 10 else "."}',
            colour = discord.Color.dark_gold()
        ).set_footer(text=f'Histórico de {member.name}, {len(infraçõesDeMembros[member.id])} linhas de log.')
        await channelsend(embed=embed)
        
    else:
        await channelsend(f'{author.mention}, que bom! Não temos registro do usuário {member}.')



@client.command(aliases = ['limparhistorico', 'limparHistorico', 'limparhistórico'])
async def limparHistórico(ctx, user: discord.Member):

    message = ctx.message
    author = message.author
    channelsend = message.channel.send

    global reg_tp, ademir, moderador, supervisor, natasid, infraçõesDeMembros
        
    if not [cargo for cargo in [ademir, moderador, supervisor] if cargo in author.roles] and not author.id == natasid:
        await channelsend(f'{author.mention}, você não tem permissão para usar este comando!'); return 

    msg = await channelsend(embed=discord.Embed(
        title = f'**{author.name}, deseja remover todas as infrações do usuário {user.name}?**',
    ...
    def check(reaction, user):                                                                              #check para verificar se os statements ali são true
        ...
        return user == author and str(reaction.emoji) == '👍' and msg.id == reaction.message.id
    try:
        reaction, user = await client.wait_for('reaction_add', timeout=15, check=check)                     #detecção padrão
    except asyncio.TimeoutError:
        await channelsend('Você demorou muito para responder, operação cancelada!', delete_after=2.5); return
    else:
        del infraçõesDeMembros[user.id]; savelista(infraçõesDeMembros, 'infraçõesDeMembros')
        await channelsend(f'{author.mention}, {user.mention} teve seu histórico limpo!')
